"""Unit tests for the version-history store behind dynamic ruleset add/update/archive."""
...
def mock_encryption_key():
    with patch("app.config.settings.decision_ledger_encryption_key", "Zle6mPbk-f2bJdNym2gB1fnW8TcvVU5ZWzzRM_Mc8kc="):
        yield
...
def test_bootstrap_from_db_loads_all_five_real_rulesets(db):
    # seed_from_directory() targets the RuleStore facade (dual-writes the authoritative
    # repository + vector index) — wrap the raw RulesetStore the same way production
    # does, then assert against the repository directly.
    from app.rule_store import RuleStore
    from app.stores.vector_index import InMemoryVectorIndex

    store = RulesetStore(db)
    rule_store = RuleStore(settings=None, repository=store, vector_index=InMemoryVectorIndex())
    seed_from_directory(rule_store, RULESETS_DIR)

    assert "de.device-financing" in store.list_ruleset_ids()
    assert store.get_active("de.device-financing")["version"] == 1
